...
bool OGLES2PhantomMask::RenderScene()
{
	...
	*/
	...
	// Shadow
	m_Print3D.Print3D(0.3f, 0, 1.02f,  PVRTRGBA(0,0,0,255), "PhantomMask");

	float fOffset =  int(m_Print3D.GetFontLineSpacing() / 4.8f) / (PVRT_MIN(PVRShellGet(prefWidth), PVRShellGet(prefHeight)) / 480.0f);

	if(m_bEnableSH)
	{
		// Shadow        
		m_Print3D.Print3D(0.3f, fOffset, 0.805f,  PVRTRGBA(0,0,0,255), "Spherical Harmonics Lighting");
	...
	{
		// Shadow
		m_Print3D.Print3D(0.3f, fOffset, 0.805f,  PVRTRGBA(0,0,0,255), "Vertex Lighting");
